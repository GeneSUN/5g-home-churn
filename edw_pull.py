...
if __name__ == "__main__":
    ...
    churn_df = query_to_edw(query)
    churn_df.show(truncate  = False)
    hdfs_pa  = 'hdfs://njbbepapa1.nss.vzwnet.com:9000'
    hdfs_pd  = 'hdfs://njbbvmaspd11.nss.vzwnet.com:9000'
    churn_df.withColumn( "MTN", F.trim( F.col("MTN") ) )\
            .withColumn( "ACTIVITY_CD", F.trim( F.col("ACTIVITY_CD") ) )\
            .write.mode("overwrite").parquet(hdfs_pd + f"/user/ZheS/5g_Churn/churn_df" ) 
    #churn_df.write.mode("overwrite").parquet(hdfs_pd + f"/user/ZheS/5g_Churn/original_churn_df" ) 
